...
const SocialsPanel = ({ socialData, setSocialData }) => {
    ...
    const openMediaLibrary = ( onSelectAction ) => {
        ...
        const frame = window.wp.media( {
            title: __( 'Select or Upload Media', 'express-social-links' ), // Updated text domain
            button: { text: __( 'Use this media', 'express-social-links' ) },
            multiple: false 
        } );
           ...
           <div style={ { display: 'flex', flexDirection: 'column', gap: '24px' } }>
                ...
                { socialData.social.map( ( row, index ) => (
                    <Flex key={ index } align="end" gap={ 4 } style={ { borderBottom: '1px solid #f0f0f0', paddingBottom: '16px', marginBottom: '8px', alignItems: 'center'} }>
                        ...
                        <FlexItem className="express-social-links-logo-column">
                            <div style={ { position: 'relative', width: '55px', height: '55px' } }>
                                { row.logo ? (
                                    <img className="express-social-links-logo" src={ row.logo } onClick={ () => openMediaLibrary( ( media ) => updateField( index, 'logo', media.url ) ) }
                            ...
                            <label className="components-base-control__label" style={ { display: 'block', marginTop: '8px', fontStyle: 'italic',color: '#50575e' } }>
                                { __( 'Socio Icon', 'express-social-links' ) }
                            </label>
                        </FlexItem>
                        <Flex className="express-social-links-field-inputs-column">
                            <FlexItem className="platform-label">
                                <TextControl
                                    label={ __( 'Social Platform Label', 'express-social-links' ) }
                                    value={ row.label } placeholder="e.g., GitHub"
                                    onChange={ ( val ) => updateField( index, 'label', val ) }
                                />
                            </FlexItem>
                            <FlexItem className="platform-url">
                                <TextControl label={ __( 'Social Platform URL', 'express-social-links' ) } value={ row.url }
                                    placeholder="https://..."
            ...
            <Flex justify="flex-start" style={ { marginTop: '12px' } }>
                <Button variant="secondary" icon={ plus } onClick={ addRepeaterRow }>
                    { __( 'Add New Social Link', 'express-social-links' ) }
                </Button>
            </Flex>
            <style>
                {`
                    ...
                    .express-social-links-logo-column {
                        align-items: center;
                        flex-basis:70px;
                        flex-grow: 0;
                    }

                    .express-social-links-logo {
                        width: 35px;
                        height: 35px;
                    }

                    .express-social-links-field-inputs-column{
                        display: flex;
