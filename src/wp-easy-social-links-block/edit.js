...
export default function Edit( { attributes, setAttributes } ) {
	...
	if ( ! globalSettings ) {
		return (
			<div { ...blockProps }>
				<Placeholder label={ __( 'Loading Global Social Options...', 'wp-easy-social-links' ) }>
					<Spinner />
				</Placeholder>
			</div>
		);
	...
	return (
		<div { ...blockProps }>
			...
			<InspectorControls>
				<PanelBody title={ __( 'Display Options', 'wp-easy-social-links' ) } initialOpen={ true }>
					<ToggleControl
						label={ __( 'Show Social Icons', 'wp-easy-social-links' ) }
						checked={ socialDataPayload.showHideSocials }
						onChange={ ( val ) => setAttributes( { showHideSocials: val } ) }
					/>
					<ToggleControl
						label={ __( 'Show Text Labels', 'wp-easy-social-links' ) }
						checked={ socialDataPayload.showText }
						onChange={ ( val ) => setAttributes( { showText: val } ) }
					/>
					<ToggleControl
						label={ __( 'Show Icon Borders', 'wp-easy-social-links' ) }
			...
			<div className="marcels-easy-socials-block-editor-canvas">
				{ ! socialDataPayload.showHideSocials ? (
					...
					<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '80px', background: '#f0f0f1', borderRadius: '6px', border: '1px dashed #ccd0d4', padding: '10px' }}>
						<p style={{ color: '#646970', fontStyle: 'italic', fontSize: '13px', margin: 0 }}>
							❌ { __( 'Social block display is toggled hidden.', 'wp-easy-social-links' ) }
						</p>
					</div>
				) : isListEmpty ? (
					// Empty Slate Fallback Notification
					<p style={{ color: '#757575', fontStyle: 'italic', fontSize: '13px', padding: '10px 0' }}>
						{ __( 'You have not added any social links yet. Add them in the block sidebar or global options.', 'wp-easy-social-links' ) }
					</p>
				) : (
